...
export const INITIAL_CSV = `Order Number,Product,Price,Date,Payment Method
 ...
 * Safely handles common CSV quotes, dollar signs, and whitespace.
 */
export function parseCSV(csvText: string): Order[] {
  ...
  // Find column indices
  // Expected headers: "Order Number", "Product", "Price", "Date", "Payment Method"
  const orderNumIndex = headers.findIndex(h => h.includes("order") || h.includes("number"));
  const productIndex = headers.findIndex(h => h.includes("product") || h.includes("item"));
  ...
  const payIndex = headers.findIndex(h => h.includes("payment") || h.includes("method"));
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Standard CSV splitter handling potential double quotes
    // (though our standard data is single-comma separated without embedded commas)
    ...
    // Get values based on detected indices or fallback to defaults
    const rawOrderNum = columns[orderNumIndex !== -1 ? orderNumIndex : 0] || `UNKNOWN-${i}`;
    const rawProduct = columns[productIndex !== -1 ? productIndex : 1] || "Unknown Product";
    const rawPrice = columns[priceIndex !== -1 ? priceIndex : 2] || "$0.00";
    const rawDate = columns[dateIndex !== -1 ? dateIndex : 3] || "2025-08-01";
    const rawPayment = columns[payIndex !== -1 ? payIndex : 4] || "Cash";

    // Clean price (remove symbols like $, commas, then parse)
    const cleanedPriceStr = rawPrice.replace(/[^\d.-]/g, "");
    const priceNum = parseFloat(cleanedPriceStr) || 0;

    parsedOrders.push({
      id: `${rawOrderNum}-${rawProduct}-${rawDate}-${i}`, // unique primary ID
...
/**
 * Packs list of Orders back into a standard formatted CSV string.
 */
export function exportToCSV(orders: Order[]): string {
  const header = "Order Number,Product,Price,Date,Payment Method";
  const rows = orders.map(o => 
    `"${o.orderNumber}","${o.product.replace(/"/g, '""')}",$${o.price.toFixed(2)},"${o.date}","${o.paymentMethod}"`
  );
